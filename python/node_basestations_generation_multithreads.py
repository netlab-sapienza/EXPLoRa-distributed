...
class myNode():
    ...
    def __str__(self):
        return "ID: "+str(self.id)+" X: "+str(self.x)+" Y: "+str(self.y)#+" PACKETS: "+str(self.packet)+" DIST: "+str(self.dist)+" ORTHO: "+str(self.ortho)
...
nrBS = 400 #400 #100 200 9 5 500 1000
nrNodes = 200000 #200000 #10000  100000 100 250000 500000
maxDist = 12000
...
matrixCordinatesDimRow = 20 #20 #15 50 3 10 23 32 9 18
matrixCordinatesDimCol = 20 #20 #15 35 3 10 23 32 9 18
...
threadPointers = []
threadOrderedOutput = [[] for i in range(numberOfThreads)]

#Create the array with the indexes to assign to each thread
splitsIndexStartEnd = []
splitsIndexStartEnd.append(0)
for i in range(1, numberOfThreads):
    splitsIndexStartEnd.append(int(nrNodes/numberOfThreads*i))
splitsIndexStartEnd.append(nrNodes)
print("Splits segments: ", splitsIndexStartEnd)

#Function that is executed by the threads
def thread_works(index, startNode, endNode):
    for i in range(startNode, endNode):
        #create the node in the space
        node = myNode(i, 0, 0)
        #Take thecoordinates of nodes already created in one array and use this to calculate the nex node spown
        nodes.append((node.x, node.y))
        #Put the nodes created in the correct position of the list of list
        threadOrderedOutput[index].append(node)

#Create the threads objects
for k in range(0, numberOfThreads):
    threadPointers.append(threading.Thread(target=thread_works, args=(k,splitsIndexStartEnd[k], splitsIndexStartEnd[k+1])))
    

#Wait fot the threads end and write on the file the outputs
numberOfStartedThreads=0
with open('simulation_files/' + str(nrNodes) + '-nodes-raw-BwGw-' + str(int(distanceBetweenGW)) + '-BwGN-' + str(maxDist) + '-mygen.txt', 'w') as nfile:
    while(numberOfStartedThreads<numberOfThreads):
        #Start N threads at a time
        for k in range(0+numberOfStartedThreads, numberOfConcurrentThreads+numberOfStartedThreads):
            threadPointers[k].start()
        #Wait for N threads to end at a time
        for k in range(0+numberOfStartedThreads, numberOfConcurrentThreads+numberOfStartedThreads):
            #Wait fot the cuurent (k) thread to end
            threadPointers[k].join()
            #Take the list corresponding to the current thread and write it on the file
            for node in threadOrderedOutput[k]:
                ...
                nfile.write("\n")
            print(str(k+1)+"/"+str(numberOfThreads)+" Done. Nodes created: "+str(int(nrNodes/numberOfThreads*k)))
            #Delete the written list to free space
            threadPointers[k] = None
            threadOrderedOutput[k]=None
            #del threadOrderedOutput[k]
        
        #Increment the total number of started and ended threads
        numberOfStartedThreads += numberOfConcurrentThreads
