...
public class EXPLoRa_C_Runnable_splitView implements Runnable{
    ...
    public EXPLoRa_C_Runnable_splitView(Stream<Node> inputNodes, int workerNumberOfNodes, int threadId) {
    ...
    public void run() {
        ...
        //Print the starting calculated distribution
        System.out.println("Thread " + threadId + ": Real calculated Waterfilling: "+Arrays.toString(maxDevicesForSpreadingfactor));
        ...
        //nodeStream.forEach(node -> {System.out.println(((Node)node).toString()); counter++;});
        //System.out.println("Thread " + threadId + ": Number of nodes to process: " + counter);
        //System.out.println("Thread " + threadId + ": Spreding factors stack: " + spreadingFactorsStack.toString());
        //System.out.println("Thread " + threadId + ": Spreading factors stack size: " + spreadingFactorsStack.size());
        ...
        outputArray = nodeStream.map(node -> {
                                    ...
                                    //For the splitView search the correct basestation ID
                                    maxRSSIindex = currentNode.getBaseStations().get(maxRSSIindex);
                    ...
                    .map(node ->{
                                    ...
                                    if(currentNode.getReferenceSpreadingFactor()==0){
                                        ...
                                        if(!availableSFsForThisNode.isEmpty()){
                                            ...
                                            System.out.println("Thread "+ threadId +": WARNING SPREADING FACTOR NOT ASSIGNED BECASUE NOT AVAILABLE, IS NEEDED TO OVERCOME THE DISTRIBUTION (EX. NODE NEED SF 12, DISTRIBUTION HAS 1 SF 12 TO ASSIGN, SF 12 ALREADY ASSIGNED)");
                                            //Assign the minmum spreading factor needed to reach the Gateway without taking into account the distribution and all the EXPLoRa-C considerations PART DONE BY MYSELF, NO RULES IN THE PAPER
                                            for(int k=0; k<sensitivityThresholds.length; k++){
                                                if(currentNode.getReferenceBaseStationRSSI() >= sensitivityThresholds[k]){
                                                    currentNode.setReferenceSpreadingFactor(k+7);
                                            ...
                                            //System.out.println(spreadingFactorsStack.toString());
                                            System.out.println(currentNode.toString());
                                            //If this happen insert a code that assign the first SF that satisfy the RSSI without taking into account the distribution
                                        }
                                    }
                        ...
                        System.err.println("Thread " + threadId + ": Remaining SFs int the stack: " +spreadingFactorsStack.toString());
                        
                        //Calculate and print the final assigned spreading fators distribution
                        int[] finalAssignedSFs = new int[6];
                        for(int i=0; i< outputArray.length; i++){
                            finalAssignedSFs[outputArray[i].getReferenceSpreadingFactor()-7] += 1;
                        }
                        System.out.println("Thread " + threadId + ": Fial Spreading Factors distribution: "+Arrays.toString(finalAssignedSFs));
